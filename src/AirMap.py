...
class airtableDataMapper(object):
    def __init__(self, baseKey:str, APIkey:str):
        ...
        def getTableJSON(tableName:str):
            """
            Grabs Airtable JSON via Airtable's API
             Parameters:
                tableName : str || Airmap table name
             Returns:
                response_json : json || Airtable API json response
            """
        ...
        self.outputNameToID = {
            x['fields']['OutputID']: x['id']
            for x in self.outputFormats['records'] }
        # table's row ID to Names
        self.sourceIDToName = {
            x['id']: x['fields']['SourceName']
            for x in self.dataSources['records'] }
        # - - - - - - - - - - - - - -

    # pulls mapping information associated with a specific view and preps mapping objects
    def findMappingView(self, viewName:str):
        """
        Finds the requested Mapping View defined in Airtable
         Parameters:
            viewName : str || Airmap view name
         Returns:
            N/A
        """
        ...
        def getTableViewJSON(tableName:str):
            """
            Grabs Airtable Table View JSON via Airtable's API
             Parameters:
                tableName : str || Airmap table name
            """
        ...
        self.inputSourceIDs = list( {
            x['fields']['InputSource'][0] for x in self.dataMap['records']
            if self.targetOutputID in x['fields']['OutputFormat'] } )
        self.inputSourceNames = [
            *map(self.sourceIDToName.get, self.inputSourceIDs ) ]
        self.outputSourceIDs = list( {
            x['fields']['OutputSource'][0]
            for x in self.dataMap['records'] if self.targetOutputID in x['fields']['OutputFormat'] } )
        self.outputSourceNames = [
            *map(self.sourceIDToName.get, self.outputSourceIDs ) ]
        ...
        chainedSourcesAPI_str = ",".join(
            [ f"DataSource='{x}'" for x in tempAllSourceNames ] )
        ...
        self.columnIDToSourceID = {
            x['id']: x['fields']['DataSource'][0] for x in self.dataColumns['records'] }
        # table's row ID to Names
        self.columnIDToName = {
            x['id']: x['fields']['ColumnName']
            for x in self.dataColumns['records'] }
        # - - - - - - - - - - - - - -

        # LISTS
        # - - - - - - - - - - - - - -
        # nested list of merge column ID pairs
        self.mergeColIDPairs = [
            x['fields']['MergeColumns']
            for x in self.dataConnectors['records'] ]
        ...
        self.inputColsInOutput = [
            x['fields']['InputColumn'][0] for x in self.dataMap['records'] 
            if (self.targetOutputID in x['fields']['OutputFormat']) 
            and (x['fields']['InputColumn'][0] in self.mergeColIDsList) ]
        # - - - - - - - - - - - - - -

        # preparations for data mapping
        map_df = pd.json_normalize(self.dataMap['records'])
        map_col_rename_dict = { 
            x: x.replace("fields.", "") for x in map_df.columns.tolist() }
        map_df = map_df.rename(columns=map_col_rename_dict)
        list_cols = [
            'OutputFormat', 'InputColumn', 'OutputColumn', 'MaxLength', 'ColumnPosition', 
            'DataType', 'Format', 'Active', 'Description', 'Required' 
            ]
        ...
        map_df['InputSource'] = map_df.loc[:, 'InputColumn'].copy().map(
            self.columnIDToSourceID).map(self.sourceIDToName)
        ...
        map_df = map_df[ [
            'OutputColumn', 'InputColumn', 'InputSource', 'MaxLength', 'ColumnPosition', 
            'DataType', 'Format', 'Active', 'Description', 'Required' ] ]
    ...
    def viewInputSources(self):
        """
        Displays Airtable Table View input sources
         Returns:
            inputSourceNames : list || list of all source names described in a given table view
        """
        return self.inputSourceNames

    def viewInputColumns(self):
        """
        Displays Airtable Table View input columns
         Returns:
            inputCols_df : pd.DataFrame || dataframe of all input columns described in a given table view
        """
    ...
    def viewMap(self):
        """
        Displays Airtable Table View map
         Returns:
            map_df : pd.DataFrame || dataframe of the actual Table View mapping
        """
        return self.map_df

    def mapData(self, data:dict):
        """
        Intakes provided data and outputs the final merged and formatted data as specified by the Airtable Table View
         Parameters:
            data : dict || dictionary of all data sources used in the data merge
         Returns:
            all_data_df : pd.DataFrame || final merged and formatted data
        """
        ...
        sourceColNames = {
            x: temp_df[temp_df['InputSource'] == x]['InputColumn'].tolist()
        ...
        mergeColsToAppend = [
            i for sub in self.mergeColIDPairs for i in sub
            for x in self.inputColsInOutput if i != x ]
        # Adds source to each mergeColsToAppend column as nested list pairs.
        mergeSourceColsToAppend = [
            [ self.sourceIDToName[self.columnIDToSourceID[x]], self.columnIDToName[x] ]
            for x in mergeColsToAppend ]
        ...
        merge_data = [
            { 'SourceToRename': self.columnIDToSourceID[i], 'Key':i, 'Value':x,
              'MergeSources':[self.columnIDToSourceID[i], self.columnIDToSourceID[x]] }
            for sub in self.mergeColIDPairs 
            for i in sub
            for x in self.inputColsInOutput 
            if i != x ]
        ...
        merge_df['SourceToRename'] = merge_df['SourceToRename'].map(
            self.sourceIDToName)
        merge_df['MergeSources'] = merge_df['MergeSources'].apply(
            lambda x: [ *map(self.sourceIDToName.get, x) ] )
        merge_df['KeyValuePair'] = [
            { key: value } for key, value in zip(
            merge_df['Key'], merge_df['Value']) ]

        # creates dict used to convert a sources merge column to the correct merge column name
        temp_df = (
            merge_df[['SourceToRename', 'KeyValuePair']]
            .copy().set_index('SourceToRename') )
        ...
        for source in sourceColNames:
            ...
            if source in mergeColConvert:
                data[source] = data[source].rename(
                    columns=mergeColConvert[source])
        ...
        all_data_df = data[mergeColAndSources[0][1][0]].merge(
            data[mergeColAndSources[0][1][1]], on=mergeColAndSources[0][0])
        mergedSourcesCheck = [mergeColAndSources[0]
                              [1][0], mergeColAndSources[0][1][1]]
